...
def eval_auc_tune(result_path, dataset='OTB2015'):
    #list_path = os.path.join(realpath(dirname(__file__)), '../../', 'dataset', dataset + '.json')
    if 'OTB' in dataset:
        list_path = os.path.join('/home/zhangxiang/Data/OTB100', dataset+'.json')
    elif 'TC' in dataset:
        list_path = os.path.join('/home/zhangxiang/Data/Temple-color-128', 'TC128'+'.json')
    elif 'UAV' in dataset:
        list_path = os.path.join('/home/zhangxiang/Data/UAV123', dataset+'.json')
    annos = json.load(open(list_path, 'r'))
    seqs = list(annos.keys())  # dict to list for py3
    n_seq = len(seqs)
    thresholds_overlap = np.arange(0, 1.05, 0.05)
