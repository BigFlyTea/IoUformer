...
from IoUnet.lib.models.backbone import FrozenBatchNorm2d
from IoUnet.lib.models.repvgg import RepVGGBlock
# import time
...
class IOU(nn.Module):
    """A simple fully connect layer to predict IOU"""
    def __init__(self, in_planes=256, out_planes=1, input_sz=8, bias=True):
...
def build_box_head(cfg):
    if cfg.MODEL.HEAD_TYPE == "IOU":
        if cfg.MODEL.BACKBONE.DILATION is False:
            stride = 16
        else:
            stride = 8
        feat_sz = int(cfg.DATA.TEMPLATE.SIZE / stride)
        channel = getattr(cfg.MODEL, "HEAD_DIM", 256)
        return IOU(in_planes=channel, input_sz=feat_sz)
