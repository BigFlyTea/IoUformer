...
class L2Problem:
    ...
    def ip_output(self, a, b):
        ...
        if (a.view(-1) @ b.view(-1)).dim() == 0:
            return torch.sum(a.view(-1) @ b.view(-1))
        return sum(a.view(-1) @ b.view(-1))
...
class ConjugateGradientBase:
    ...
    def run_CG(self, num_iter, x=None, eps=0.0):
        ...
        if self.debug:
            ...
            resvec[0] = normr
        ori_normr = self.residual_norm(r)
        ...
        for ii in range(num_iter):
            ...
            if eps > 0.0 or self.debug:
                normr = self.residual_norm(r)
                if normr<cfg.TEST.NORM_THRESHOLD*ori_normr:
                    #print('Stopped CG since norm smaller than THRESHOLD')
                    break
            if self.debug:
