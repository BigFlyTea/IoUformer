...
import math
import numpy as np
from classifier.optim import ConvProblem, FactorizedConvProblem
from classifier.libs import TensorList, operation
from classifier.libs import fourier, augmentation, dcf
from classifier.libs.plotting import show_tensor
...
class BaseClassifier(object):
    ...
    def initialize(self, image, state, input_size, scale):

        # Initialize some stuff
        self.frame_num = 1

        #self.img_sample_sz = torch.round(torch.Tensor([cfg.TRACK.INSTANCE_SIZE, cfg.TRACK.INSTANCE_SIZE]))
        self.img_sample_sz = torch.round(torch.Tensor([input_size, input_size]))
        if 'alex' in cfg.META_ARC:
            l = cfg.TRACK.INSTANCE_SIZE // cfg.ANCHOR.STRIDE - cfg.ANCHOR.STRIDE - 1
        else:
            l = self.img_sample_sz // cfg.MODEL.TOTAL_STRIDE
        self.output_sz = torch.round(torch.Tensor([input_size, input_size]))
        self.feature_sz = TensorList([torch.Tensor(l)])
        self.kernel_size = TensorList([cfg.TEST.KERNEL_SIZE])
        #self.score_sz = cfg.TRAIN.OUTPUT_SIZE

        # Get all geometry information
        self.pos = torch.Tensor([state[1] + (state[3] - 1)/2, state[0] + (state[2] - 1)/2])
        #self.pos = torch.Tensor(input_size/2)
        ...
        # hinge mask
        self.use_attention_layer = cfg.TEST.USE_ATTENTION_LAYER and (cfg.TEST.CHANNEL_ATTENTION or cfg.TEST.SPATIAL_ATTENTION != 'none')

        # Optimization options
        self.precond_learning_rate = cfg.TEST.LEARNING_RATE
        self.CG_forgetting_rate = cfg.TEST.CG_FORGETTING_RATE
    ...
    def init_optimization(self, train_x, init_y):
        ...
        # Get parameters
        self.update_projection_matrix = cfg.TEST.UPDATE_PROJECTION_MATRIX and cfg.TEST.USE_PROJECTION_MATRIX
        optimizer = cfg.TEST.OPTIMIZER

        # Setup factorized joint optimization
        if self.update_projection_matrix:
            self.projection_reg = TensorList([cfg.TEST.PROJECTION_REG])
            ...
            plot_show = cfg.TEST.DEBUG_CLASS and cfg.TEST.VISUALIZE_CLASS
            if optimizer == 'GaussNewtonCG':
                self.joint_optimizer = GaussNewtonCG(self.joint_problem, joint_var, plotting=plot_show, analyze=cfg.TEST.ANALYZE_CONVERGENCE, fig_num=(12, 13, 14))
            ...
            if isinstance(cfg.TEST.INIT_CG_ITER, (list, tuple)):
                self.joint_optimizer.run(cfg.TEST.INIT_CG_ITER)
            else:
                self.joint_optimizer.run(cfg.TEST.INIT_CG_ITER // cfg.TEST.INIT_GN_ITER, cfg.TEST.INIT_GN_ITER)

            if cfg.TEST.ANALYZE_CONVERGENCE:
        ...
        print(self.training_samples[0].shape)
        ji
        if optimizer == 'GaussNewtonCG':
            self.filter_optimizer = ConjugateGradient(self.conv_problem, self.filter, fletcher_reeves=cfg.TEST.FLETCHER_REEVES,
        ...
        # Post optimization
        self.filter_optimizer.run(cfg.TEST.POST_INIT_CG_ITER)
    ...
    def track(self, scale_idx=None):
        ...
        #self.feat_x tensor list feat_x[0] [1,64,45,45]
        # Compute scores
        scores_raw = self.apply_filter(self.feat_x)
        #scores_raw tensor list scores_raw[0] [1,1,45,45]
        s, flag = self.localize_target(scores_raw)

        if cfg.TEST.VISUALIZE_CLASS:
    ...
    def update(self, state, scale_z, flag):
        ...
        learning_rate = cfg.TEST.HARD_NEGATIVE_LEARNING_RATE if hard_negative else cfg.TEST.LEARNING_RATE
        #self.scale = scale_z
        if update_flag:
            #!!! the label is not corresponding to feat_x !!!#
        ...
        # Train filter
        if hard_negative:
            self.filter_optimizer.run(cfg.TEST.HARD_NEGATIVE_CG_ITER)
        elif (self.frame_num-1) % cfg.TEST.TRAIN_SKIPPING == 0:
    ...
    def localize_target(self, scores_raw):
        ...
        if cfg.TEST.ADVANCED_LOCALIZATION:
    ...
    def localize_advanced(self, scores):
        ...
        if max_score1.item() < cfg.TEST.TARGET_NOT_FOUND_THRESHOLD:
            return scores, 'not_found'

        # Mask out target neighborhood
        target_neigh_sz = cfg.TEST.TARGET_NEIGHBORHOOD_SCALE * self.target_sz * self.scale
        ...
        if max_score2 > cfg.TEST.DISTRACTOR_THRESHOLD * max_score1:
            disp_norm1 = torch.sqrt(torch.sum(target_disp1**2))
            disp_norm2 = torch.sqrt(torch.sum(target_disp2**2))
            disp_threshold = cfg.TEST.DISPLACEMENT_SCALE * math.sqrt(sz[0] * sz[1])  / 2
        ...
        if max_score2 > cfg.TEST.HARD_NEGATIVE_THRESHOLD * max_score1 and max_score2 > cfg.TEST.TARGET_NOT_FOUND_THRESHOLD:
            return scores, 'hard_negative'
    ...
    def project_sample(self, x: TensorList, proj_matrix = None):
        ...
        if self.use_attention_layer:
            ...
            if cfg.TEST.SPATIAL_ATTENTION == 'none':
            ...
            elif cfg.TEST.SPATIAL_ATTENTION == 'pool':
    ...
    def init_learning(self):
        ...
        # Filter regularization
        self.filter_reg = TensorList([cfg.TEST.FILTER_REG])
        ...
        att_activation = cfg.TEST.ATT_ACTIVATION
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        aug_expansion_factor = cfg.TEST.AUGMENTATION_EXPANSION_FACTOR
        ...
        self.random_shift_factor = cfg.TEST.RANDOM_SHIFT_FACTOR
        ...
        if cfg.TEST.AUGMENTATION_SHIFT:
            self.transforms.extend([augmentation.Translation(shift, aug_output_sz) for shift in cfg.TEST.AUGMENTATION_SHIFT])
        if cfg.TEST.AUGMENTATION_RELATIVESHIFT:
            get_absolute = lambda shift: (torch.Tensor(shift) * self.img_sample_sz/2).long().tolist()
            self.transforms.extend([augmentation.Translation(get_absolute(shift), aug_output_sz) for shift in cfg.TEST.AUGMENTATION_RELATIVESHIFT])
        if cfg.TEST.AUGMENTATION_FLIPLR:
            self.transforms.append(augmentation.FlipHorizontal(aug_output_sz, self.get_rand_shift()))
        if cfg.TEST.AUGMENTATION_BLUR:
            self.transforms.extend([augmentation.Blur(sigma, aug_output_sz, self.get_rand_shift()) for sigma in cfg.TEST.AUGMENTATION_BLUR])
        if cfg.TEST.AUGMENTATION_SCALE:
            self.transforms.extend([augmentation.Scale(scale_factor, aug_output_sz, self.get_rand_shift()) for scale_factor in cfg.TEST.AUGMENTATION_SCALE])
        if cfg.TEST.AUGMENTATION_ROTATE:
            self.transforms.extend([augmentation.Rotate(angle, aug_output_sz, self.get_rand_shift()) for angle in cfg.TEST.AUGMENTATION_ROTATE])

        # Generate initial samples
        init_samples = self.extract_transformed_samples(im, self.transforms)

        # Remove augmented samples for those that shall not have
        for i, use_aug in enumerate(TensorList([cfg.TEST.USE_AUGMENTATION])):
            if not use_aug:
                init_samples[i] = init_samples[i][0:1, ...]

        # Add dropout samples
        if cfg.TEST.AUGMENTATION_DROUPOUT:
            num, prob = cfg.TEST.AUGMENTATION_DROUPOUT
    ...
    def init_projection_matrix(self, x):
        ...
        self.use_projection_matrix = cfg.TEST.USE_PROJECTION_MATRIX

        if self.use_projection_matrix:
            self.compressed_dim = TensorList([cfg.TEST.COMPRESSED_DIM])
            proj_init_method = cfg.TEST.PROJ_INIT_METHOD
    ...
    def init_label_function(self, train_x):
        ...
        self.y = TensorList([x.new_zeros(cfg.TEST.SAMPLE_MEMORY_SIZE, 1, x.shape[2], x.shape[3]) for x in train_x])
        # Output sigma factor
        output_sigma_factor = cfg.TEST.OUTPUT_SIGMA_FACTOR
        ...
        for y, sig, sz, ksz, x in zip(self.y, self.sigma, self.feature_sz, self.kernel_size, train_x):
            ...
            for i, T in enumerate(self.transforms[:x.shape[0]]):
                sample_center = center_pos + torch.Tensor(T.shift) / self.img_sample_sz * sz ##calculate center bias
    ...
    def init_memory(self, train_x):
        ...
        self.sample_weights = TensorList([x.new_zeros(cfg.TEST.SAMPLE_MEMORY_SIZE) for x in train_x])
        for sw, init_sw, num in zip(self.sample_weights, self.init_sample_weights, self.num_init_samples):
            sw[:num] = init_sw

        # Initialize memory
        self.training_samples = TensorList(
            [x.new_zeros(cfg.TEST.SAMPLE_MEMORY_SIZE, cdim, x.shape[2], x.shape[3]) for x, cdim in
    ...
    def update_sample_weights(self, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, learning_rate = None):
        ...
        for sw, prev_ind, num_samp, num_init in zip(sample_weights, previous_replace_ind, num_stored_samples, num_init_samples):
            ...
            init_samp_weight = cfg.TEST.INIT_SAMPLES_MINIMUM_WEIGHT
