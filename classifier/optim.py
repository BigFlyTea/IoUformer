import torch
from classifier.libs import optimization, TensorList, operation
import math
...
class FactorizedConvProblem_v2(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y:TensorList, kernel:torch.Tensor,
    ...
    def __call__(self, x: TensorList):
        ...
        #compressed_samples = operation.conv1x1(self.training_samples, P).apply(self.projection_activation)
        # do first hadamard product
        kernel_0 = operation.matmul(self.kernel, filter).apply(self.projection_activation)

        # Do second convolution
        #residuals = operation.conv2d(compressed_samples, filter, mode='same').apply(self.response_activation)
        # do second hadamard product
        kernel_1 = operation.matmul(kernel_0, P).apply(self.response_activation)

        # do convolution on sample.
        residuals = operation.conv2d(self.training_samples, kernel_1)
...
class ConvProblem(optimization.L2Problem):
    ...
    def __call__(self, x: TensorList):
        ...
        residuals = operation.conv2d(self.training_samples, x, mode='valid')
